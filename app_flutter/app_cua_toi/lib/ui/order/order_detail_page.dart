...
class _OrderDetailPageState extends State<OrderDetailPage> {
  ...
  // Hàm xử lý chung cho Confirm, Cancel và Complete
  void _handleAction(String actionType) async {
    ...
    switch (actionType) {
      case 'CONFIRM':
        response = await OrderService.confirmOrder(widget.orderId);
        break;
      case 'CANCEL':
        response = await OrderService.cancelOrder(widget.orderId);
        break;
      case 'COMPLETE':
        response = await OrderService.completeOrder(widget.orderId);
        break;
    }
    ...
    if (mounted) {
      if (response.code == 1000) {
        ...
        _fetchDetail(); // Reload trang chi tiết
        OrderService.fetchMyOrders(); // Reload danh sách bên ngoài
      } else {
        setState(() => _isLoading = false);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text("${response.message}. Vui lòng thử lại!"),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    // Logic hiển thị nút
    bool showPendingActions = _order!.status == 'PENDING';
    bool showCompleteAction =
        _order!.status == 'DELIVERED'; // Chỉ hiện khi đã giao
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            _buildSection(child: _buildHeaderInfo()),
            const SizedBox(height: 12),
            _buildSection(child: _buildReceiverInfo()),
            const SizedBox(height: 12),
            _buildSection(child: _buildProductList()),
            const SizedBox(height: 12),
            _buildSection(child: _buildPaymentInfo()),

            // --- KHU VỰC NÚT HÀNH ĐỘNG ---

            // 1. Nút cho trạng thái PENDING
            ...
            // 2. Nút cho trạng thái DELIVERED (Hoàn thành)
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: getStatusColor(_order!.status)),
              ),
              child: Text(
                _order!.status,
                style: TextStyle(
  ...
  Widget _buildReceiverInfo() {
    return Column(
      ...
      children: [
        ...
        Text(_order!.phoneNumber, style: TextStyle(color: Colors.grey[700])),
        const SizedBox(height: 4),
        Text(_order!.fullAddress, style: TextStyle(color: Colors.grey[700])),
  ...
  Widget _buildPaymentInfo() {
    return Column(
      ...
      children: [
        ...
        _rowInfo("Tổng tiền hàng", _formatCurrency(_order!.totalAmount)),
